...
void loop() {
  ...
  if(sample_timer.isup()) {
    ...
    pkt.ax = voltage_to_millig(ax_raw.avg());
    pkt.ay = voltage_to_millig(ay_raw.avg());
    pkt.az = voltage_to_millig(az_raw.avg());

    pkt.gx = voltage_to_millig(gx_raw.avg());
    pkt.gy = voltage_to_millig(gy_raw.avg());
    pkt.gz = voltage_to_millig(gz_raw.avg());

    pkt.crc = computeCRC((uint8_t*)&pkt, sizeof(IMUPacket) - 1);

    Serial.write((uint8_t*)&pkt, sizeof(IMUPacket));
