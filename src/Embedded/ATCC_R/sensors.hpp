...
ADCSensor SusPotRearHeave(5,0,1000); // verified, works (RR in old harness)
// ADCSensor test46(6,0,1000);
// ADCSensor test47(7,0,1000);

// ADCSensor test50(0,0,1000);
// ADCSensor test51(1,0,1000);
ADCSensor RotorTempRR(2,500,5); // verified, works
// ADCSensor test53(3,0,1000);
// ADCSensor test54(4,0,1000);
// ADCSensor test55(5,0,1000);
ADCSensor RotorTempRL(6,500,5);
ADCSensor SusPotRearRoll(7,0,1000); // modify to include this (RL in old harness)
...
void sample_ADCs()
{
  ...
  if (sample_timer_1.isup())
  {
    ...
    adc4.sample(SusPotRearHeave);
    adc5.sample(RotorTempRR, RotorTempRL, SusPotRearRoll);
    // adc6.sample(test60, test61, test62, test63, test64, test65, test66, test67);
  }
  
}

float voltage_to_sus_pot_rear_heave_val(float voltage) {
  float max_travel = 50.0;
  return (voltage / 5.0) * max_travel;
}

// float voltage_to_sus_pot_rear_roll_val(float voltage) {
//   float max_travel = 0;
//   return (voltage / 5.0) * max_travel;
// }
