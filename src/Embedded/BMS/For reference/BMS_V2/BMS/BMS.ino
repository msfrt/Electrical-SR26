...
void setup() {
  ...
  Serial.print("wake ping sent again\n");
  ...
  bqWriteReg(0, FAULT_RST1, 0xFF, 1, FRMWRT_STK_W); //Reset faults on stacked devices
  bqWriteReg(0, FAULT_RST2, 0xFF, 1, FRMWRT_STK_W); //Reset faults on stacked devices
  bqWriteReg(0, Bridge_FAULT_RST, 0x22, 1, FRMWRT_SGL_W); //Reset FAULT_COMM and FAULT_SYS on bridge device

  //ENABLE BQ79616-Q1 MAIN ADC
  bqWriteReg(0, DEV_CONF2, 0x0C, 1, FRMWRT_STK_W); //set all cells to active
  bqWriteReg(0, ADC_CTRL2, 0x05, 1, FRMWRT_STK_W);   //continuous run and ADC_GO (bq79718)
  //SpiWriteReg(0, ADC_CTRL2, 0x06, 1, FRMWRT_STK_W);   //continuous run and MAIN_GO (given for 616)
  delayus(5*TOTALBOARDS + 192);                       //5us reclocking per board and 192us for round robin to complete
  
  bqReadReg(0, Bridge_DEBUG_SPI_FRAME, response_frame, 2, 0, FRMWRT_SGL_R);
  int point = 0;
  for(int i = 0; i <2; i++){
    point = response_frame[i];
    Serial.print(point);
...
void loop() {
  ...
  if (millis() - lastBlink > 500) { // Blink every 500ms
    ...
    ledState = !ledState;
    digitalWrite(13, ledState);
    // bqReadReg(0, Bridge_DEBUG_SPI_FRAME, response_frame, 1, FRMWRT_SGL_R, 10);
    point = 0;
    for(int i = 0; i <3; i++){
      point = response_frame[i];
      Serial.print(point);
