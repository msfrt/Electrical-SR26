...
BMSErrorCode_t bqGetAllCellVoltages(BMSOverallData_t *bmsData) {
    ...
    BMSErrorCode_t status = bqStackRead(start_addr, rawVoltageData, CELLS_PER_SLAVE * 2, SERIAL_TIMEOUT_MS * NUM_BQ79718_DEVICES * 2);
    // // Print rawVoltageData as hex bytes for debugging
    // for (size_t idx = 0; idx < CELLS_PER_SLAVE * 2 * NUM_BQ79718_DEVICES; ++idx) {
    ...
    for (int i = 0; i < NUM_BQ79718_DEVICES; ++i) {
        for (int j = 0; j < CELLS_PER_SLAVE; ++j) {
            ...
            // BQ79718 voltage data is 16-bit signed, MSB first in memory map (e.g. VCELLx_HI then VCELLx_LO)
...
BMSErrorCode_t bqGetAllTemperatures(BMSOverallData_t *bmsData) {
    ...
    uint8_t rawTempData[(TEMP_SENSORS_PER_SLAVE * 2 + 6) * NUM_BQ79718_DEVICES] = {0};

    BMSErrorCode_t status = bqStackRead(GPIO1_HI, rawTempData, TEMP_SENSORS_PER_SLAVE * 2, SERIAL_TIMEOUT_MS * NUM_BQ79718_DEVICES);
    if (status != BMS_OK) {
    ...
    for (uint16_t cb = 0; cb < NUM_BQ79718_DEVICES; cb++) {
    ...
    status = bqStackRead(DIETEMP1_LO, rawTempData, 2 * NUM_BQ79718_DEVICES, SERIAL_TIMEOUT_MS * NUM_BQ79718_DEVICES);
    if (status != BMS_OK) {
...
BMSErrorCode_t bqGetStackFaultStatus(BMSOverallData_t *bmsData) {
    uint8_t faultSummaryBuffer[NUM_BQ79718_DEVICES]; 

    BMSErrorCode_t status = bqStackRead(FAULT_SUMMARY, faultSummaryBuffer, 1, SERIAL_TIMEOUT_MS * NUM_BQ79718_DEVICES);
    if (status != BMS_OK) {
        BMS_DEBUG_PRINTLN("Failed to read stack fault summaries.");
        for(int i=0; i < NUM_BQ79718_DEVICES; ++i) bmsData->modules[i].fault_summary = 0xFF; 
        return status;
    }
    ...
    for(int i=0; i < NUM_BQ79718_DEVICES; ++i) {
        ...
        if (faultSummaryBuffer[i] != 0) {
            // BMS_DEBUG_PRINTF("Fault summary on BQ79718 module %d (Addr %d): 0x%02X. Reading details...\n", i, deviceAddress, faultSummaryBuffer[i]);
            ...
            if(bqReadReg(deviceAddress, FAULT_OT1, &(bmsData->modules[i].fault_ot), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_ot=0xFF;
            if(bqReadReg(deviceAddress, FAULT_UT1, &(bmsData->modules[i].fault_ut), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_ut=0xFF;
            if(bqReadReg(deviceAddress, FAULT_COMM, &(bmsData->modules[i].fault_comm1), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_comm1=0xFF;
            if(bqReadReg(deviceAddress, FAULT_COMM+1, &(bmsData->modules[i].fault_comm2), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_comm2=0xFF;
            if(bqReadReg(deviceAddress, FAULT_COMM+2, &(bmsData->modules[i].fault_comm3), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_comm3=0xFF;
            if(bqReadReg(deviceAddress, FAULT_SYS, &(bmsData->modules[i].fault_sys), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_sys=0xFF;
            if(bqReadReg(deviceAddress, FAULT_OC, &(bmsData->modules[i].fault_prot1), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_prot1=0xFF;
            if(bqReadReg(deviceAddress, FAULT_OC+1, &(bmsData->modules[i].fault_prot2), 1, FRMWRT_SGL_R) != BMS_OK) bmsData->modules[i].fault_prot2=0xFF;
        } else {
...
uint8_t estimateSOC(const BMSOverallData_t *bmsData) {
    if (NUM_BQ79718_DEVICES == 0 || CELLS_PER_SLAVE == 0) return 0;
    if (bmsData->totalPackVoltage_mV == 0) return 0; // Avoid division by zero if pack voltage isn't read yet

    float avgCellVoltage_mV = (float)bmsData->totalPackVoltage_mV / (NUM_BQ79718_DEVICES * CELLS_PER_SLAVE);
...
BMSErrorCode_t bqStopCellBalancing(BMSOverallData_t *bmsData) {
    ...
    BMSErrorCode_t status = bqWriteReg(0, BAL_CTRL2, 0x01, 1, FRMWRT_STK_W);
    if (status == BMS_OK) {
        // Optionally clear tracking for all modules
        for (uint8_t i = 0; i < NUM_BQ79718_DEVICES; ++i) {
...
BMSErrorCode_t bqCheckBalancingStatus(uint8_t moduleIndex, BMSOverallData_t *bmsData) {
    if (moduleIndex >= NUM_BQ79718_DEVICES) return BMS_ERROR_UNKNOWN;
    ...
    status = bqReadReg(deviceAddress, BAL_DONE1, cb_complete_buf, 2, FRMWRT_SGL_R);
    if (status != BMS_OK) {
...
void printBQDump() {
    ...
    // ... (rest of BQ79718 dump remains the same, uses `read_val_buf` and correct B0_reg.h names) ...
    ...
    for (uint8_t i = 0; i < NUM_BQ79718_DEVICES; ++i) {
        uint8_t deviceAddress = i + 1; 
        BMS_DEBUG_PRINTF("\nBQ79718 Device %d (Addr 0x%02X) Registers:\n", i, deviceAddress);
        ...
        // ... (Include all other BQ79718 registers as previously listed, using read_val_buf)
        status = bqReadReg(deviceAddress, DEV_CONF1, read_val_buf, 1, FRMWRT_SGL_R);
        ...
        if (status == BMS_OK) BMS_DEBUG_PRINTF("  FAULT_SYS:   0x%02X\n", read_val_buf[0]); else BMS_DEBUG_PRINTLN("  FAULT_SYS:   READ FAILED");
        status = bqReadReg(deviceAddress, FAULT_COMM, read_val_buf, 1, FRMWRT_SGL_R);
        if (status == BMS_OK) BMS_DEBUG_PRINTF("  FAULT_COMM1: 0x%02X\n", read_val_buf[0]); else BMS_DEBUG_PRINTLN("  FAULT_COMM1: READ FAILED");

        status = bqReadReg(deviceAddress, FAULT_COMM+1, read_val_buf, 1, FRMWRT_SGL_R);
        if (status == BMS_OK) BMS_DEBUG_PRINTF("  FAULT_COMM2: 0x%02X\n", read_val_buf[0]); else BMS_DEBUG_PRINTLN("  FAULT_COMM2: READ FAILED");
        ...
        status = bqReadReg(deviceAddress, FAULT_OT1, read_val_buf, 1, FRMWRT_SGL_R);
        if (status == BMS_OK) BMS_DEBUG_PRINTF("  FAULT_OT:    0x%02X\n", read_val_buf[0]); else BMS_DEBUG_PRINTLN("  FAULT_OT:    READ FAILED");

        status = bqReadReg(deviceAddress, FAULT_UT1, read_val_buf, 1, FRMWRT_SGL_R);
...
void send_pack_data(const BMSOverallData_t *d)
{
    ...
    // for (uint8_t m=0; m<NUM_BQ79718_DEVICES; ++m) {
    ...
    for (uint8_t mod = 0; mod < NUM_BQ79718_DEVICES; ++mod)
    {
        Serial.printf("\nModule %u\n", mod);
        // voltages
        Serial.print("Volt :");
