...
void SpiAutoAddress(){
    ...
    uint16_t randomData[3] = {0x12, 0x2F, 0xC5};
    SPI1.beginTransaction(SPISettings(BRIDGE_FREQ, LSBFIRST, SPI_MODE0));
    digitalWrite(CS1, LOW);
    for (int i=0; i<3; i++){
        SPI1.transfer(randomData[i]);
    }
    SPI1.endTransaction();
    digitalWrite(CS1, HIGH);
    Serial.print("SPI_RDY ==");
    ...
    bqReadReg(0, 0x2120, devIDFrame, 2, FRMWRT_STK_R, 10);
    
    Serial.print("CONTROL1 read: ");
    for(int i=0; i < 16; i++){
        if(devIDFrame[i] < 0x10) Serial.print("0");
        Serial.print(devIDFrame[i]);
    }
    ...
    bqWriteReg(0, COMM_CTRL, 0X00, 1, FRMWRT_ALL_W); // (clear [TOP_STACK] information after communication direction is changed.)

    bqWriteReg(0, CONTROL1, 0X01, 1, FRMWRT_ALL_W); // (enable stack device auto addressing)
    ...
    bqWriteReg(TOTALBOARDS-1, COMM_CTRL, 0x01, 1, FRMWRT_SGL_W); // set highest addr board as top of stack
